...
var ChartEdit = function () {
    ...
    $("#options-div .list-group-item > a").on('click',function(){
        ...
        // if($("#chart_type").val() == 4 || $("#chart_type").val() == 10){
    ...
    $("#option-tooltipdiv .tooltip-text").on('blur',function(){ 
...
function line_stacked_draw(content, idx){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            var tooltipsHTML = "";
            $("#option-tooltipdiv").html("");
            if((ctxData != "") && (chartData != null)){
                chartData.tooltip.forEach((item, index) => {
                    tooltipsHTML = "<label>Tooltip" + (index+1) + ":</label><input onblur='tooltipBlur()' type='text' class='form-control tooltip-text' style='width:100%;'>";
                    $("#option-tooltipdiv").append(tooltipsHTML);
                    $(".tooltip-text")[index].value = item;
                });   
            }
            ...
            if((ctxData != "") && (chartData != null)){
                chartData.backgroundColor.forEach((item, index) => {
                    chartHTML = "<div class='color-set'><label>Label:</label><input onblur='labelBlur()' type='text' class='form-control label-text' style='width:100%;'><label>Color:</label><input onchange='colorBlur()' type='color' class='form-control chartcolorpicker' style='width:100%;'></div>";
                    $("#option-chartdiv").append(chartHTML);
                    $(".chartcolorpicker")[index].value = item;
                    $(".label-text")[index].value = chartData.chart[index];
                }); 
            }
            ...
            if((ctxData != "") && (chartData != null)){
                if(chartData.tooltip.length == 0){
                    var tempHTML = "";
                    for(var k = 0; k < dCount; k++){
                        tempHTML = "<label>Tooltip" + (k+1) + ":</label><input onblur='tooltipBlur()' type='text' class='form-control tooltip-text' style='width:100%;'>";
                        $("#option-tooltipdiv").append(tempHTML);
                    }     
                }
            }
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(chartData != null){
                ...
                myChart.data.datasets.forEach((dataset, index) => {
                    dataset.label = chartData.chart[index];
                });
...
function pie_draw(content, idx){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            var data = generateData(content, tmpColorArr);            
            dCount = data.datasets.length;
            config = {
                type: type,
                data: data,
...
function colorBlur(){
    myChart.data.datasets.forEach((item, index) => {
        item.backgroundColor = $(".chartcolorpicker")[index].value;
